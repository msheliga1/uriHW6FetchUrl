    ...
    fetch(requestUrl)
        ...
        .then(function (data) {
            ...
            if (data.cod >= 300) {
                ...
                // addNoResulsFoundToDom("No movies found for superhero " + heroName);
                return;
            }
            ...
            var requestUrl2 = apiUrlBase + forecastText + latlonQS + appIdQS;
            myLog("Open weather map lat-lon request url: " + requestUrl2);
            ...
            .then(function (data) {
                if (response.cod >= 300) {
                    myLog("Could not find data for lat-long ");
                    return;
                }
                console.log(data);  // this shows up in console as collaspable-expandable. Better than stringify.
                if (data.total_results === 0) {
                            myLog("No weather results found for " + cityName);
                            // addNoResulsFoundToDom("No movies found for superhero " + heroName);
                            return;
                    }
            })
